...
import ExamCard from '../components/exams/ExamCrad';
...
const ExamsPage = () => {
  ...
  const filteredExams = activeCategory === 'all'
    ? examsData
    : examsData.filter(exam => exam.category.toLowerCase() === activeCategory);
      ...
      <div className="container mx-auto px-4 py-4">
        <div className="md:flex md:items-center md:justify-between">
          <div className="flex items-center space-x-1 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
            {['all', 'government', 'entrance'].map((cat) => (
              <button
                key={cat}
              ...
              >
                {cat === 'all' ? 'All Exams' : cat.charAt(0).toUpperCase() + cat.slice(1) + ' Exams'}
              </button>
            ))}
          </div>
